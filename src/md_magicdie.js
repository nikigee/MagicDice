	...
	class diceRoll {
		...
		render() {
			...
			this.list.forEach((x, i) => {
				...
				x.list.forEach((v, i) => {
					htmlList += `<div class="gfx_dice">${v}</div>${
						i + 1 == x.stats.iterator ? "" : "+"
					}`;
				});
		...
		render(callback) {
			...
			this.fields.forEach((v, k) => {
				if (v.type == 0) {
                    ...
                    <input type="text" placeholder="${v.content}" class="${
						this.ID + k
					}">`;
				} else if (v.type == 2) {
					content += `
		...
		if (magicHandler.managed_players.length && device_width > 436) {
			...
			for (let i = 0; i < magicHandler.managed_players.length; i++) {
				...
				toolbar.insertAdjacentHTML(
					"beforeend",
					`<div class="toolbar-hero"><span>${initials}</span><i class="fa fa-user-circle"></i><i class="fa fa-pencil" aria-hidden="true"></i><i class="fa fa-book"></i><i class="fa fa-sticky-note"></i><i class="fa fa-keyboard-o"></i><i class="fa fa-floppy-o"></i><i class="fa fa-cloud"></i><i class="fa fa-trash-o"></i></div>`
				);
				document
					.getElementsByClassName("toolbar-hero")
					[i].getElementsByClassName("fa-pencil")[0]
				...
				document
					.getElementsByClassName("toolbar-hero")
					[i].getElementsByClassName("fa-book")[0]
					.addEventListener("click", (e) => {
						magicHandler.managed_players[i].render.spellbook.generate();
					});
				document
					.getElementsByClassName("toolbar-hero")
					[i].getElementsByClassName("fa-floppy-o")[0]
					.addEventListener("click", (e) => {
						magicHandler.managed_players[i].saveToFile();
					});
				document
					.getElementsByClassName("toolbar-hero")
					[i].getElementsByClassName("fa-cloud")[0]
					.addEventListener("click", (e) => {
						magicHandler.managed_players[i].save();
					});
				document
					.getElementsByClassName("toolbar-hero")
					[i].getElementsByClassName("fa-trash-o")[0]
					.addEventListener("click", (e) => {
				...
				document
					.getElementsByClassName("toolbar-hero")
					[i].getElementsByClassName("fa-keyboard-o")[0]
					.addEventListener("click", (e) => {
